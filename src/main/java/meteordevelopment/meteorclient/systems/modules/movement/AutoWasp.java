    ...
    public void onActivate() {
        if (target == null || target.isRemoved()) {
            target = (Player) TargetUtils.get(entity -> {
                ...
                return !onlyFriends.get() || Friends.get().get((Player) entity) != null;
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (!mc.player.isFallFlying()) {
            ...
            if (jumpTimer >= 4) {
                ...
                mc.getConnection().send(new ServerboundPlayerCommandPacket(mc.player, net.minecraft.network.protocol.game.ServerboundPlayerCommandPacket.Action.START_FALL_FLYING));
            }
        } else {
            incrementJumpTimer = false;
            jumpTimer = 0;
    ...
    private void onMove(PlayerMoveEvent event) {
        ...
        if (avoidLanding.get()) {
            ...
            for (Direction dir : DirectionAccessor.meteor$getHorizontal()) {
                ...
                if (((BlockBehaviourAccessor) mc.level.getBlockState(pos).getBlock()).meteor$isHasCollision() && Math.abs(targetPos.y() - (pos.getY() + 1)) <= 0.25) {
