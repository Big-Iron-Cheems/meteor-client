    ...
    private void onRender(Render3DEvent event) {
        ...
        if (firedProjectiles.get()) {
            for (Entity entity : mc.level.entitiesForRendering()) {
                if (entity instanceof Projectile && (!ignoreWitherSkulls.get() || !(entity instanceof WitherSkull))) {
    ...
    private class Path {
        ...
        private void processHitResult(HitResult result) {
            if (result.getType() == HitResult.Type.BLOCK) {
                ...
                points.add(Utils.set(vec3s.get(), result.getLocation()).add(0, collidingEntity.getBbHeight() / 2, 0));
            }
        }
